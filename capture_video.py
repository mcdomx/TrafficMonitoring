...
def get_frame(stats: dict) -> (int, np.array, float):
...
def update_window(d_delay: float, elapsed_time: ElapsedTime):
    """
...
def stream_object_detection():
    ...
    while True:
        ...
        print("Elapsed time: {:<8} Buffer size: {:<3} delay: {}".format(round(elapsed_time.get(), 1), qs.ref_queue.qsize(), delay), end='\r')
